...
const columns = [
    ...
    { key: "Statut_Employé", label: "Statut Employé" },
    { key: "Poste_Employé", label: "Poste Employé" },
    { key: "type_Contrat", label: "Type Contrat" },
    { key: "type_credit", label: "Type Crédit" },
    { key: "montant_Credit", label: "Montant demandé" },
    { key: "Statut_Credit", label: "Statut Crédit" },
];
...
const CreditHistoriqueTable = forwardRef((props, ref) => {
    const {
        departementId,
    ...
    console.log("List filtres :",preloadedEmployees);
    ...
    const getUniqueValues = (key) => {
        if (!filtered) return [];
        return [...new Set(filtered.map(item => console.log(item[key])).filter(val => val !== null && val !== undefined && val !== ""))].sort((a, b) => {
    ...
    const uniqueMatricules = useMemo(() => getUniqueValues('TypeContrat'), [filtered]);
    const uniqueCins = useMemo(() => getUniqueValues('Anciennete'), [filtered]);
    const uniquePostes = useMemo(() => getUniqueValues('poste'), [filtered]);
    const uniqueNumerosCimr = useMemo(() => getUniqueValues('salaire'), [filtered]);
    const uniqueDatesAff = useMemo(() => getUniqueValues('TypeCredit'), [filtered]);
    const uniqueDatesFin = useMemo(() => getUniqueValues('MontantDemande'), [filtered]);
    const uniqueSalaires = useMemo(() => getUniqueValues('nbr_mois'), [filtered]);
    const uniqueTaux = useMemo(() => getUniqueValues('TauxApplique'), [filtered]);
    const uniqueMontants = useMemo(() => getUniqueValues('MensualiteEstimee'), [filtered]);
    ...
    const onSave = (newData, isFormData = false) => {
        ...
        if (isFormData) {
            ...
            if(editingItem){
                request = axios.post(`http://127.0.0.1:8000/api/credit/${editingItem.credits[0]?.id_credit}?_method=PUT`, newData, config)
                request.then(res => {
                    Swal.fire({
                        icon: 'success',
                        title: 'Succès',
                ...
                request = axios.post("http://127.0.0.1:8000/api/historique", newData, config);
            }
        }
    };
    ...
    const columns_config = useMemo(() => [
        ...
        {   key: 'Poste_Employé', 
            label: "Poste Employé", 
            visible: columnVisibility.poste,
            render: (row) => row.fonction || "-"
        },

        {   key: 'type_Contrat', 
            label: "Type Contrat", 
            visible: columnVisibility.TypeContrat ,
...

        {
            ...
            visible: columnVisibility.MontantDemande,
                            ...
                            <div className="d-flex align-items-center justify-content-between flex-wrap" style={{ gap: '16px' }}>
                                <div style={{ flex: '1 1 300px', minWidth: 0 }}>
                                    ...
                                    <p className="section-description text-muted mb-0">
                                        {searched.length} historique {searched.length > 1 ? 's' : ''} affichée{searched.length > 1 ? 's' : ''}
                                    </p>
                                </div>
                                ...
                                <div style={{ display: "flex", gap: "10px", alignItems: 'center', flexWrap: 'wrap' }}>
                                    ...
                                    {/* Button des graphes d'historique */}
                                    <button
...
export default CreditHistoriqueTable;
