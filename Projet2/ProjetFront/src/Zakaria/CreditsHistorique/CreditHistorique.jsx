...
import CreditHistoriqueTable from "./CreditHistoriqueTable";
...
function HistoriqueManager() {
    ...
    const [preloadedEmployees, setPreloadedEmployees] = useState(() => {
        const cached = localStorage.getItem('historiqueCredit');
        return cached ? JSON.parse(cached) : [];
    });

    console.log(preloadedEmployees);
        ...
        axios.get("http://127.0.0.1:8000/api/credit")
            .then(res => {
                if(res.data && Array.isArray(res.data)){
                    const allValidCredits = res.data.flatMap(emplo =>
                        emplo.credits
                            .filter(credit => 
                                credit.statut &&
                                credit.statut.toLowerCase().trim() !== "en_attente"
                            )
                            .map(credit => ({
                                id: credit.id_credit,
            ...
            setPreloadedEmployees(allValidCredits);
            localStorage.setItem('historiqueCredit', JSON.stringify(allValidCredits));
        }
            })
