...
function FinanceManager(){
    ...
    //l'importation des credits pour le valider
    const [preloadedCredits,setPreloadedCredits] = useState(() => {
        const Credits = localStorage.getItem('ValidationCredit');
        ...
        axios.get("http://127.0.0.1:8000/api/credit")
            .then(res => {
                if(res.data && Array.isArray(res.data)){
                    const fetchCredits = res.data.flatMap(employe => 
                        employe.credits
                            .filter(credit => 
                                credit.statut &&
                                credit.statut === "Validé (R.H)"
                            )
                            .map(credit => ({
                                id: credit.id_credit,
                                ...
                                type_contrat: employe.contrats?.[0]?.type_contrat,
                                statut_emp: Number(employe.active) ? "Actif" : "Suspendu",
                                fonction: employe.fonction,
                                salaire: employe.salaire_base,
                                ...
                                duree_demande: credit.nbr_mois,
                                statut_credit: credit.statut,
                                taux_credit: credit.type_credit?.taux_typeCredit

                            }))
                    );

                    setPreloadedCredits(fetchCredits);
                    localStorage.setItem('employesCredit',JSON.stringify(fetchCredits))
                }
            });
    }
