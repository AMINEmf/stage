...
function CreditManager() {
    ...
    // Préchargement des employés pour CreditTable
    const [preloadedEmployees, setPreloadedEmployees] = useState(() => {
        const cached = localStorage.getItem('employesCredit');
    ...
    //l'importation des départements et le sauvgarder dans localStorage
        ...
        axios.get("http://127.0.0.1:8000/api/credit")
            .then(res => {
                if(res.data && Array.isArray(res.data)){
                    const emp = res.data.flatMap(emplo =>
                        emplo.credits
                            .filter(credit => 
                                credit.statut &&
                                ["en_attente","validé (finance)" ,"validé (r.h)","refusé"].includes(credit.statut.toLowerCase().trim())
                            )
                            .map(credit => ({
                                id: credit.id_credit,
                                id_typeCredit: credit.type_credit?.id_typeCredit,

                                employe: `${emplo.nom} ${emplo.prenom}`,
                                departement: emplo.departement.nom,
                                anciennete: emplo.anciennete,
                                statut_Emp: Number(emplo.active) ? "actif" : "Suspendu",
                                salaire_base: emplo.salaire_base,
                                fonction: emplo.fonction,
                                type_contrat: emplo.contrats?.[0]?.type_contrat,

                                montant_credit: credit.montant_credit,
                                mensualite_credit: parseFloat(credit.mensualite),
                                statut_credit: credit.statut,
                                type_credit: credit.type_credit?.nom_typeCredit,
                                date_credit: credit.date_credit,

                                departement_id: emplo.departement_id,
                                id_employe: credit.id_employe,
                        }))
                    )

                    setPreloadedEmployees(emp);
                    localStorage.setItem('employesCreditBackup',JSON.stringify(emp))
                }
            })
            .catch(err => console.error("Error preloading credit:", err));
    }
    ...
    useEffect(() => {
        ...
        // Précharger les employés pour CreditTable (en parallèle)
        fetchCredits()
                    ...
                    <div className="departement_home1">
                        ...
                        {contextMenu.visible && (
                            <div className="context-menu" style={{ top: `${contextMenu.y}px`, left: `${contextMenu.x}px` }}>
                                <button onClick={() => handleAddEmployeClick(contextMenu.departementId)}>Ajouter un crédit</button>
                                <button
